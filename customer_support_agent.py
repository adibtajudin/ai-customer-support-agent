...
if groq_api_key:
    ...
    class CustomerSupportAIAgent:
        ...
        def handle_query(self, query, user_id=None, chat_history=None, customer_profile=None):
            # Use the raw customer profile JSON as source of truth
            system_prompt = (
                "You are a customer support AI agent for TechGadgets.com, an online electronics store. "
                "Answer questions using ONLY the exact details in the customer profile below. "
                "Never mix up products, prices, or dates between different orders. "
                "The recentOrder field in the profile is always the customer's latest order — use it for any order-related questions unless a specific order number is mentioned.\n\n"
            )

            if customer_profile:
                system_prompt += f"Customer Profile (source of truth):\n{json.dumps(customer_profile, indent=2)}"
    ...
    if st.sidebar.button("Generate Synthetic Data"):
        if customer_id:
            with st.spinner("Clearing old data and generating fresh customer profile..."):
                support_agent.clear_memories(customer_id)
    ...
    if st.sidebar.button("View Memory Info"):
        if customer_id:
            ...
            if memories:
                ...
                for mem in memories:
                    text = mem.get("memory") or mem.get("text", "") if isinstance(mem, dict) else mem
    ...
    if query and customer_id:
        ...
        answer = support_agent.handle_query(
                query,
                user_id=customer_id,
                chat_history=st.session_state.messages,
                customer_profile=st.session_state.get("customer_data")
            )
    ...
    st.warning("Please enter your Groq API key to use the customer support agent.")
